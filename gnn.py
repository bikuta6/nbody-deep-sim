...
def transform_to_graph(positions, features, y, batch=None, neighbors=50, device='cuda'):
    ...
    data = Data(x=torch.cat((positions, features), dim=-1), edge_index=graph, edge_attr=None, y=y, batch=batch)

    return data


class GraphModel(torch.nn.Module):
    def __init__(self, input_dim=1, output_hiddens=None, output_dim=3, node_encoder_dims=None, gnn_dim=128, encoder_dropout=0.0, message_passing_steps=4, 
                 aggr='sum', device='cpu',neighbors=50):
    ...
    def train_batch(self, optimizer, pos, feat, acc):
        ...
        batch = torch.cat([torch.full((pos[i].size(0),), i, dtype=torch.long, device=self.device) for i in range(len(pos))])
    ...
    def predict(self, pos, feat):
        self.eval()
        with torch.no_grad():
            data = transform_to_graph(pos, feat, torch.zeros((pos.size(0), 3), device=self.device), device=self.device)
